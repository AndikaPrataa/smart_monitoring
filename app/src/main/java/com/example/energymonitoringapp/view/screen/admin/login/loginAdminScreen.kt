package com.example.energymonitoringapp.view.screen.admin.login
...
fun LoginAdminScreen(
    ...
    Column(
        ...
        LoginForm(
            ...
            onLoginClick = {
                if (validate()) {
                    authViewModel.login(email = email, password = password, expectedRole = SessionManager.ROLE_ADMIN)
                }
            }
        )
...
fun LoginAdminScreenPreview() {
    EnergyMonitoringAppTheme { LoginAdminScreen() }
}
